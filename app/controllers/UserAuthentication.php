...
class UserAuthentication {
  ...
  const EN_MANTENIMIENTO= false;
    ...
    if ( self::EN_MANTENIMIENTO == true ){
			View::render("maintenance");// para un mejor UI, No usar esta vía. Copia todo /views/maintenance.php y pégalo momentáneamente en /index.html
			return;
		}
		...
		if( isset($user->usuario) ) { /* && is_object($user->usuario)) */
			...
			Transaccion::insertTransaccionLogin("Not_Ok", $username, "<MD5>", "", $user_agent);
	...
	public function forget(){
		...
		$forgetPassword = Utils::sanitize( stripslashes($forgetPassword), 40 );

		$user = UserAdmin::getUserByEmailOrUsername($forgetPassword);

		if( isset($user->usuario) ) {
      /* SI encontrado */
      $boolUpdated = UserAdmin::establecerPasswordTemporal( $user );
      ...
      if ( $boolUpdated ){
        ...
        if ($sent == true){
          ...
          View::set("pageTitle", "Revise su correo");
   ...
   * @return JSON format
   */
  public function get_operandos_matematicos(){

    $operandosObj = Utils::randomOperation();

    $return_arr = array(
      "op1" =>      Utils::getHtmlValue( $operandosObj->getOp1() ),
      "operator" => Utils::getHtmlValue( $operandosObj->getOperator() ),
      "op2" =>      Utils::getHtmlValue( $operandosObj->getOp2() ),
      "t" =>        base64_encode( "xzy" . $operandosObj->getResult()  . "qwerty") //la funcion que la decodifica ha sido ofuscada con https://obfuscator.io/ | la original se conserva en Lanuza Group - pendientes.txt
    );
