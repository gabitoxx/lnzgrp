...
class Utils {
   ...
   * @param length - tamaño máximo del campo permitido en la BD
   */
  public static function sanitize($text, $length){
		
    $n = trim($text);
    $n = strip_tags($n); //strip_tags() function strips a string from HTML, XML, and PHP tags.

    /*
      $cadena = ' texto  <Gabriel>Dario<script lang="javascript"><b>neg</b>alert("hey");function delete { DELETE FROM USERS;}</script> fuera ';
      $patrones = array();
      $patrones[0] = '/(<[\W\w]+>)+/i';
      $sustituciones = array();
      $sustituciones[2] = '';

      $r = preg_replace($patrones, $sustituciones, $cadena);    borraria desde el primer '<' hasta el ultimo '>'

    */
    $r = str_ireplace("\"", "", $n); //case-insensitive search | otherwise use str_replace()
    $r = str_ireplace("'", "", $r);
    $r = str_ireplace("`", "", $r);
    $r = str_ireplace("(", "", $r);
    $r = str_ireplace(")", "", $r);
    ...
    $r = str_ireplace("{", "", $r);
    $r = str_ireplace("}", "", $r);
    $r = str_ireplace("DELETE", "", $r);
    $r = str_ireplace("INSERT", "", $r);
    $r = str_ireplace("UPDATE", "", $r);
    $r = str_ireplace("SELECT", "", $r);
    $r = str_ireplace("FROM", "", $r);
    
    if ( $length > 0 && strlen($r) > $length ){
      $r = substr($r, 0, $length);
    }
    return $r;
  }
  ...
  /**
   * Ejecutar una Operacion Random
   * @return Operation objeto
   */
  public static function randomOperation(){
    $i = mt_rand(1,5);
    if( $i == 1 || $i == 3) { return Utils::sum();}
    else if( $i == 2 ) { return Utils::res();}
    else if( $i == 4 ) { return Utils::mul();}
    else { return Utils::div();}
  }
  
  private static function sum(){  return Utils::sumAlg(true); }
  private static function res(){  return Utils::sumAlg(false);}

  private static function sumAlg($esSuma){
    $array1 = array(9,8,5,6,7);
    $array2 = array(3,1,2,4,5);
    $i = mt_rand(1,5) - 1;
    $j = mt_rand(1,5) - 1;
    $op1 = $array1[$i];
    $op2 = $array2[$j];
    ...
    $pojo = new Operation( $op1, $op2, $y, $f );
    return $pojo;
  }

  private static function mul(){
    $array1 = array(2,3,1,4,1);
    $array2 = array(1,2,4,3,1);
    $i = mt_rand(1,5) - 1;
    $j = mt_rand(1,5) - 1;
    $op1 = $array1[$i];
    $op2 = $array2[$j];
    $f   = $op1 * $op2;
    $pojo = new Operation( $op1, $op2, "*", $f );
    return $pojo;
  }
  private static function div(){
    $array1 = array(4,8,12,8,4);
    $array2 = array(1,2,2,4,1);
    $i = mt_rand(1,5) - 1;
    $j = mt_rand(1,5) - 1;
    $op1 = $array1[$i];
    $op2 = $array2[$j];
    $f   = $op1 / $op2;
    $pojo = new Operation( $op1, $op2, "div", $f );
    return $pojo;
  }

  /**
   * Obtiene el valor a mostrar en el HTML
   */
  public static function getHtmlValue($op){
    if ( $op == 1 ){
      return '<svg xmlns="http://www.w3.org/2000/svg" class="operando" viewBox="0 0 50 50" stroke="#000" stroke-linecap="round" stroke-linejoin="round" fill="none" fill-rule="evenodd" style="stroke-width:2"><path d="M29.5-.5v50"/><path d="M19.5 49.5h20m-20-40l10-10"/></svg>';
    } else if ( $op == 2 ){
      return 'deux.png';
    } else if ( $op == 3 ){
      return 'troix.png';
    } else if ( $op == 4 ){
      return '<svg xmlns="http://www.w3.org/2000/svg" class="operando" viewBox="0 0 50 50" stroke="#000" stroke-linecap="round" stroke-linejoin="round" fill="none" fill-rule="evenodd" style="stroke-width:2"><path d="M29.5-.5v50m0-50l-20 30"/><path d="M9.5 29.5h30"/></svg>';
    } else if ( $op == 5 ){
      return 'cinq.png';
    } else if ( $op == 6 ){
      return 'seize.png';
    } else if ( $op == 7 ){
      return '<svg xmlns="http://www.w3.org/2000/svg" class="operando" viewBox="0 0 50 50" stroke="#000" stroke-linecap="round" stroke-linejoin="round" fill="none" fill-rule="evenodd" style="stroke-width:2"><path d="M35.9705.3519l-12.941 48.2963"/><path d="M19.5 29.5h15m-14-29h15"/></svg>';
    } else if ( $op == 8 ){
      return '<svg xmlns="http://www.w3.org/2000/svg" class="operando" viewBox="0 0 50 50" stroke="#000" stroke-linecap="round" stroke-linejoin="round" fill="none" fill-rule="evenodd" style="stroke-width:2"><ellipse cx="26.5" cy="12" rx="10" ry="7.5"/><circle cx="27" cy="32" r="12.5"/></svg>';
    } else if ( $op == 9 ){
      return '<svg xmlns="http://www.w3.org/2000/svg" class="operando" viewBox="0 0 50 50" stroke="#000" stroke-linecap="round" stroke-linejoin="round" fill="none" fill-rule="evenodd" style="stroke-width:2"><path d="M14.5 4.5h25v20h-25z"/><path fill="none" d="M39.5 24.5v15h-25"/></svg>';
    } else if ( $op == "plus" ){
      return '<svg class="operando" viewBox="0 0 50 50"><line x1="20" y1="0" x2="20" y2="40" style="stroke:rgb(255,0,0);stroke-width:2" /><line x1="0" y1="20" x2="40" y2="20" style="stroke:rgb(255,0,0);stroke-width:2" />Sorry, your browser does not support inline SVG.</svg>';
    } else if ( $op == "min" ){
      return '<svg xmlns="http://www.w3.org/2000/svg" class="operando" viewBox="0 0 50 50" stroke="#000" stroke-linecap="round" stroke-linejoin="round" fill="none" fill-rule="evenodd" style="stroke-width:2"><line x1="0" y1="20" x2="40" y2="20" style="stroke:rgb(255,0,0);stroke-width:2" />Sorry, your browser does not support inline SVG.</svg>';
    } else if ( $op == "*" ){
      return 'asterisk.png';
    } else if ( $op == "div" ){
      return '<svg xmlns="http://www.w3.org/2000/svg" class="operando" viewBox="0 0 50 50" stroke="#000" stroke-linecap="round" stroke-linejoin="round" fill="none" fill-rule="evenodd" style="stroke-width:2"><line x1="0" y1="20" x2="40" y2="20" style="stroke:rgb(255,0,0);stroke-width:2" /><circle cx="20" cy="10" r="2" stroke="red" stroke-width="4" fill="yellow" />  <circle cx="20" cy="30" r="2" stroke="red" stroke-width="4" fill="yellow" />Sorry, your browser does not support inline SVG.</svg>';
    } else {
      return $op;
    }
  }
