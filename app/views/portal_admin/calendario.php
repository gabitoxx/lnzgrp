...
<div class="col-sm-offset-1 col-sm-11">
	<h4>
		<span class="glyphicon glyphicon-calendar" style="color:<?= RGB_ADMIN; ?>; font-size:18px; "></span> 
				...
				<?php foreach($week->days() as $day): ?>
					...
					<?php 
						...
						if ( $day->isToday() ){
							...
							/* solo el Admin puede crear citas en el MISMO día */
							$enElPasado = "false";
						}
...
<?php
	...
	echo " var modalAjaxURL6 = '" . PROJECTURLMENU . "tecnicos/ajax_json_tecnicos';" ;
...
<script>
	...
	var empresasYaSeBuscaron = false;
			...
			if ( empresasYaSeBuscaron == false ){
				...
				$.ajax({
					...
					success: function(message){
						...
						$('#tecnicoAcargo').append(message);
						
					},
					error: function(){
						alert("Error al buscar info de Técnicos en nuestro Sistema\nPor favor, intente más tarde");
					}
				});
			}
	...
	function crearSoporte(){
		...
		if ( $("#tecnicoAcargo").val() == "none" ){
			bool = false;
			document.getElementById("tecnicoAcargo-div").className = "form-group has-error has-feedback";
		} else {
			document.getElementById("tecnicoAcargo-div").className = "form-group";
		}
	...
	function aceptarCita(){
		...
		var ask = confirm("¿Seguro de ACEPTAR esta cita agendada de Soporte Programado?"
			+ "\n\nRecuerde que al hacer esto se acuerda que el Ing. de Soporte seleccionado deberá ASISTIR presencialmente a dicha Empresa"
...
<!-- ====================  Opcion de CREAR una nueva cita para Soportes Programados  ========================================= -->
			...
			<form class="form-horizontal" data-toggle="validator" role="form" id="crear_soporte_form"
 			 method="post" enctype="multipart/form-data">
				...
				<div class="row" id="tecnicoAcargo-div">
					<div class="col-sm-2" align="right">
						Ing. de Soporte
					</div>
					<div class="col-sm-9">
						<div class="input-group">
							...
							<select class="form-control" id="tecnicoAcargo" name="tecnicoAcargo" onblur="javascript:return false;">
								<option value="none">-- No asignar un Ingeniero de Soporte por ahora -- </option>
							</select>
						</div>
					</div>
					<div class="col-sm-1">&nbsp;</div>
